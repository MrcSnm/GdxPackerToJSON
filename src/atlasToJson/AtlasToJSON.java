...
/**
 * @author: Marcelo Silva Nascimento Mancini(aka Hipreme/MrcSnm)
...
public class AtlasToJSON {
   ...
   public static String checkExceptions(String[] args) 
   {
      ...
      if (argc == 0) {
         ...
         filename = args[0];
         if (!CrossPlatformFunctions.checkFileExistence(filename)) {
            System.out.println("File named " + CrossPlatformFunctions.convertDirToOS(filename) + " does not exists");
            return "";
         }
   ...
   public static boolean fileCreation(String content, String filename) {
      return fileCreation(content, filename, "");
   }
   ...
   public static void main(String[] args) {
      if (args.length == 0) {
         ...
         if (CrossPlatformFunctions.checkFileExistence(jsonFile) && JOptionPane.showConfirmDialog(null,
                     CrossPlatformFunctions.convertDirToOS("File named '" + jsonFile + "' already exists in path: "
         ...
         String fileContent = fileRead(filename);
         if (fileContent != "")
            fileCreation(fileContent, jsonFile, CrossPlatformFunctions.convertDirToOS(outputFolder + '\\'));
         if (willTryToOpenExplorer)
            CrossPlatformFunctions
                        .openCurrentSystemExplorer(CrossPlatformFunctions.convertDirToOS(outputFolder + '\\'));
         if (!willTryToOpenExplorer)
            pressEnterToExit();
